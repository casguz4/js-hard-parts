...
const map = (array, callback) => {
	const arr = [];
  for (let el of array) arr.push(callback(el));
  return arr;
};
...
// Challenge 7
/*
	return an new array of elements found in every input array
*/
const intersection = (...arrays) => {
  const findMatches = (arr1, arr2) => {
    const matches = [];
    for (let el of arr2) if (arr1.indexOf(el) > -1) matches.push(el);
    return matches;
  }
	return reduce(
    arrays,
    (arr1, arr2) => {
      const matches = [];
      for (let el of arr2) {
        if (arr1.indexOf(el) > -1) matches.push(el);
      }
      return matches;
    },
    arrays[0]
...
const union = (...arrays) => {
    ...
    (arr, currentArr) => {
      const [...total] = arr;
      forEach(currentArr, el => { if (total.indexOf(el) < 0) total.push(el)});
      return total;
    },
    arrays[0]
  );
...
const multiMap = (arrVals, arrCallbacks) => {
  ...
  for (let i = 0; i < arrVals.length; i++) {
    const key = arrCallbacks[i](arrVals[i]);
    obj[key] = arrVals[i];
  }
...
const commutative = (func1, func2, value) => {
	const r1 = func2(func1(value));
  const r2 = func1(func2(value));
  return r1 === r2;
};
...
const objFilter = (obj, callback) => {
  ...
  Object.entries(obj)
    .forEach(([key, val]) => {
    	const output = callback(key);
    	if (output === val) myObj[key] = val;
  	})
  return myObj;
};
...
const rating = (arrOfFuncs, value) => {
  ...
  for (let func of arrOfFuncs) {
    if (func(value) === true) output.push(true);
  }
  return (output.length / arrOfFuncs.length) * 100;
};
...
const highestFunc = (objOfFuncs, subject) => {
	let result = 0;
  let _key;
  for (const [key, val] of Object.entries(objOfFuncs)) {
